...
from random import uniform , triangular ,normalvariate , randint , gauss,random 
...
class MTSP_DataGenerator: 
    ...
    def getInstance(self,ret=None):
        ...
        #     nodes =[ Node(uniform(0,1),uniform(0,1))  for i in range(self.node_num) ] 
        # else : 
    ...
    def node_feature(self,nodes:Node):
        node_feature = np.zeros( (self.node_num,2) , dtype=np.float32 )
    ...
    def node_DistanceMatrix(self,nodes): 
        ...
        for u , src_node in enumerate(nodes) : 
            ...
            for v , dst_node in enumerate(nodes) : 
                ...
                distance = MTSP_DataGenerator.distance_calculate(src_node,dst_node)
    ...
    def Instance_to_PMPO(self, instance): 
        ...
        PMPO_node_list = [ PMPO_node_argument(init_x=x , init_y=y ) for x ,y  in instance.x ]
