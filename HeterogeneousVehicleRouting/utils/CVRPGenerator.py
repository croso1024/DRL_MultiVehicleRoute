...
from random import uniform , triangular ,normalvariate,gauss , betavariate ,random , randint
...
class Demand_Node(object): 
    ...
    def __init__(self,x,y , demand=None): 
        ...
        self.demand = random_demand() if demand == None else demand
    def node_attribute(self)->tuple : 
        return (self.x,self.y,self.demand)
        
    

class CVRP_DataGenerator: 
    ...
    def getInstance(self,ret=None):
        ...
        Distance_matrix , edge_index , edge_attr = self.node_DistanceMatrix(nodes) 
    ...
    def node_feature(self,nodes:Demand_Node):
    ...
    def node_DistanceMatrix(self,nodes): 
        ...
        for u , src_node in enumerate(nodes) : 
            ...
            for v , dst_node in enumerate(nodes) : 
                ...
                distance = CVRP_DataGenerator.distance_calculate(src_node,dst_node)
        ...
        return Distance_matrix , edge_index , edge_attr
    ...
    def getInstance_PMPO_batch(self,ret=None):
        ...
        if random() > 0.1 :
            PMPO_node_list = [ PMPO_node_argument( init_x = uniform(0,1) , init_y = uniform(0,1) , demand=random_demand() ) for i in range(self.node_num) ] 
        else : 
            cluster_nodes = self.RandomClusteringNode()
            #PMPO_node_list = [ PMPO_node_argument(init_x=x , init_y= y , demand = demand) for x,y,demand in cluster_nodes ]
            PMPO_node_list =  [ PMPO_node_argument(init_x=node.x , init_y= node.y , demand = node.demand) for node in cluster_nodes ]
    ...
    def dataset_to_PMPOdataset_SingleProcess(self,validation_set,maximum_batch_size=None): 
        ...
        if not maximum_batch_size : 
            ...
            print(f"DEbug maximum_batch_size : {maximum_batch_size} {self.batch_size}")
    ...
    def RandomClusteringNode(self,center_var=0.07 ): 
        ...
        for i in range(self.node_num) : 
            ...
            node_x = gauss( centers[cluster_ith][0] , center_var )
            node_y = gauss( centers[cluster_ith][1] , center_var )
            node = Demand_Node(node_x , node_y , demand=None) 
            nodes.append(node) 
        return nodes 
    
    def RandomGaussDistribution(self,std=0.25): 
        # 產生中心點在(0.5,0.5) , 標準差為std的節點分佈
        center = (gauss(0.5, sigma=0.05) , gauss(0.5,sigma=0.05))
        nodes = []
        for i in range(self.node_num): 
            node_x = min(1, max(0,gauss( center[0] , sigma= std)))
            node_y = min(1, max(0,gauss( center[1] , sigma= std)))
            node = Demand_Node(node_x , node_y , demand=None) 
            nodes.append(node)
        return nodes
