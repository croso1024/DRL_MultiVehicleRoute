...
########## Parameters ############## 
num_epoch , per_dataset_size , batch_size = 100, 100 , 96
...
scheduler = StepLR(optimizer=optimizer , step_size=per_dataset_size//grad_cummulate, gamma = 0.985)
scaler = GradScaler()
...
def training_one_epoch(dataset): 
    ...
    for episode , (n,v , batch) in tqdm(enumerate(dataset)) : 
        ...
        #print(f"N:{n} , V:{v}")
        env = env_maker(batch_size , batch ,node_num=n , vehicle_num=v   ,StateEQ="PMPO",training=True) 
...
def training(): 
    ...
    score_logger_filename = f"HVRP_PMPO384_n{node_nums}_v{vehicle_nums}.npy"
    print(f"Save logger score:{score_logger}") 
...
def MixBatchGenerator():
    ...
    for setting in NV_table:  
        ...
        Batches = Training_Generator(setting[0]).getInstance_BatchPMPO_SingleProcess(dataset_size= per_dataset_size//len(NV_table))
