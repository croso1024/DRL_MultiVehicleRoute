...
class GraphNeuralNetwork(nn.Module): 
    ...
    def __init__(self,node_hidden_dim , edge_dim , heads=4,skip_connection=False,num_layers=3 ): 
        ...
        self.GraphConvolution = nn.ModuleList(
           ...
           [GraphConv_layer(hidden_dim=node_hidden_dim ) for i in range(num_layers)]
        )

        #self.JumpingKnowledge = JumpingKnowledge(mode="lstm" ,channels = node_hidden_dim , num_layers=num_layers )
        ...
        self.JumpingKnowledge_MLP = GraphSequential(
            "x,batch_ptr" , [
                (nn.Linear(num_layers*node_hidden_dim , node_hidden_dim) , "x -> x") , 
    ...
    def forward(self,node,edge_index , edge_attr , batch_ptr ): 
        ...
        return  self.JumpingKnowledge_MLP(self.JumpingKnowledge(out) , batch_ptr) 
        #return self.JumpingKnowledge_Linear(self.JumpingKnowledge(out))
        # return node
        return self.JumpingKnowledge(out)
