...
""" 
    Inference版本主要要解決在驗證或畫圖版本的時候 , 調用init_node會出現index out of range的bug , 
    因為在DSE驗證的時候會出現車輛跑到兩個節點中間，因此init_node的位置實際上不代表車子的位置 , SC_inference版本主要是修改了計算路徑長的部份,
    採用了初始化環境的時候使用的位置 ,
...
class DSVRP_Environment : 
    ...
    def calculate_splited_tour_length(self,batch_idx , splited_route): 
        """ 
            0511 : 在可以指定非節點位置初始化的情況下 , 原始SC的計算方式(假定初始車輛都在1-v的節點上)是有大錯的 , 
            SC_inference版本修正此錯誤 , 並使用在SC-CASE Verification 
        """
        ...
        if self.stateEQ_mode in ["DE","mix"] : 
            with torch.no_grad(): 
                for vehicle , path in enumerate(splited_route): 
                    ...
                    # 0511 修改 , 專用給SC inference

                    init_x , init_y = self.init_pos[batch_idx][ self.premutation_tensor[batch_idx][vehicle]   ]
                    first_x , first_y , ret = self.static_state[batch_idx][path[0]]
                    first_distance = sqrt( pow( (init_x-first_x) ,2 ) + pow( (init_y-first_y) ,2 )    )
                    # 0612: 由於實際的距離是在RouteSimulator所決定 , 因此在env裡選擇node_route_cost_std最低沒有實際幫助 ,
                    # 改成使用node_route_cost 讓模型去選擇平均距離最低 
                    distance = self.batch.node_route_cost_std[batch_idx][path[:-1],path[1:]] 
                    # distance = self.batch.node_route_cost[batch_idx][path[:-1],path[1:]] 
                    
                    
                    path_distance_cost[vehicle] = torch.sum(distance) +  ( first_distance * self.first_step_penality )
        else: 
            with torch.no_grad() : 
                for vehicle , path in enumerate(splited_route): 
