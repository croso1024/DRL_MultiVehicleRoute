...
應該要更動的部份: 
    1. reset時的vehicle_init_pos , 直接使用random mode即可  , (note init-pos =/= depot-pos ) , 要用envTest做測試 
    2. batch-data除了node_route_cost , 還得有一個node-route-std

    給路徑隨機長度的來源可以從generator就生成或者在Graph-transformer添加 ,
    效果差不多的情況下思考後用Graph-transformer後添加比較乾脆方便, 但DataGenerator就添加比較方便使用OR-tools做比較(或著OR-tools就不考慮隨機性 , 讓ORtools不知道隨機性)

    為了OR-tools使用方便 , 以及PMPO,DE各類inference方法可以統一 , 目前決定使用DataGenerator來產生節點間的隨機性, 遵守以下幾個原則
    -- 這個std的大小和兩節點間原始的距離有關 ex.   std of edge =  normal( distance , common-std )
    -- 在給edge_attr的時候 , 所有edge都是給上式的std of edge , #TODO 要能在通過KNNorRadius後保留這些std-edge    --> 在Generator的時候的edge_attr就是有std , 被KNN or Radius覆蓋掉後, 再根據索引覆蓋回來就好
    -- 除了給出node_route_cost , 另外還要有node_route_cost_withSTD , 這是"在Generator時就依照edge_attr取樣"的結果 , 確保模型在計算時會有一致的距離矩陣 
    (node route cost可以給OR-tools用在沒有考慮隨機的情況 , 也可以給OR-tools node route cost with STD 看看差距)
    
    最後就是要做實驗測試看看這種給edge_attr一部分而已的情況下模型到底能不能做出合理的預測 , 還是說特徵會不足
...
class DSVRP_Environment : 
    ...
    def reset(self): 
        ...
        if self.vehicle_pos_mode == "Depot": 
            ...
            # Random + inference-mode : 就固定使用 1 - vehicle_num的節點 , 用在validation
            else : 
                random_index = torch.repeat_interleave(
        ...
        if self.training : 
            ...
            random_capacity = torch.normal(size=(self.batch_size//random_num , self.vehicle_num) ,
                                           mean=0.7 , std=0.15 , dtype=torch.float32 , device=self.device)
