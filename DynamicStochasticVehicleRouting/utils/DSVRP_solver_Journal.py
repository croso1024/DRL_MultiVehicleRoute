""" 
    投稿版本的OR-Tools solver , 因為發現了簽前版本在計算容量上的Bug , OR-Tools的車輛不會扣除那些自身所在位置的需求 ,
    因此在計算容量上OR-Tools較為吃虧 , 這邊在外層Verification那邊把車輛所在位置的demand歸0  , 並把歸0的部份作為fix_demand傳入 
    
    這個fix_demand是用來修復原本滿足率的計算錯誤 , 要加在分子與分母 , " 目前只有修在solution_by_route這個函數 "
"""
...
class ORtools_DSVRP(object): 
    ...
    def __init__(self,dist_matrix ,dist_matrix_std ,demand_vector , vehicle_num , capacity_vector
                 , start_pos , depot_index=0 ,fix_demand = None,algo="GREEDY_DESCENT",time_limit=None):
    ...
    def solution_cost(self , solution): 
        ...
        #  完成的需求 + 一開始所在節點的容量  /  真正的完整demand , 即讓depot=0後的總需求量
        fulfill_rate = (total_load + self.fix_demand) / (sum(self.demand_vector) + self.fix_demand)
