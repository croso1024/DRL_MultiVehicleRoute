...
class GraphNeuralNetwork(nn.Module): 
    ...
    def __init__(self,node_hidden_dim , edge_dim , heads=4,skip_connection=False,num_layers=4 ): 
        ...
        self.MLP = GraphSequential(
            "x , batch_ptr"  ,  
            [
                (Linear(node_hidden_dim * num_layers, node_hidden_dim , weight_initializer="kaiming_uniform") ,"x->x"  ) ,
    ...
    def forward(self,node,edge_index , edge_attr , batch_ptr ): 
        ...
        for Convolution_layer in self.GraphConvolution : 
            ...
            hierachy_feature.append(node) 
        hierachy_feature = torch.cat(hierachy_feature,dim=1)
        # print(f"Debug hierachy shape : {hierachy_feature.shape}")
        return self.MLP(hierachy_feature , batch_ptr)
