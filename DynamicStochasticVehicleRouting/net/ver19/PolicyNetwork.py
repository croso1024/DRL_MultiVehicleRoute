...
from net.ver19.Embedding import Node_Embedding , Fleet_Embedding , Vehicle_Embedding
from net.ver19.GraphNeuralNetwork import GraphNeuralNetwork
from net.ver19.Attention import Attention_Block
from net.ver19.PermuteLayer import Permute

class PolicyNetwork(nn.Module): 
    
    def __init__(self , node_feature_dim , fleet_feature_dim , vehicle_feature_dim , hidden_dim  ,edge_dim=1, 
        ...
        self.GraphNeuralNetwork = GraphNeuralNetwork(
            node_hidden_dim=hidden_dim,edge_dim=edge_dim ,
            skip_connection=skip_connection , num_layers=num_layers , 
        )
        
        self.Pointer = PointerNetwork(hidden_dim=hidden_dim,heads=heads , temp=temp , clip_coe=clip_coe)
...
class PointerNetwork(nn.Module):
    ...
    def __init__(self,hidden_dim , heads , temp=1, clip_coe=5 ): 
        super().__init__() 
        self.hidden_dim = hidden_dim 
        self.heads = heads 
        self.clip_coe = clip_coe 
        ...
        nn.init.xavier_uniform_(self.MLP[6].weight)
        nn.init.constant_(self.MLP[6].bias,0)
    ...
    def forward(self,Graph_state , Fleet_state , Vehicle_state , batch_ptr , mask): 
        ...
        Graph_readout = self.Graph_Fleet_Attention(Graph_readout,Fleet_state,Fleet_state)
        ...
        Query = self.Vehicle_Graph_Attention4(Query,Graph_state,Graph_state)
        # Query : batch x 3 x hidden_dim --> batch x 1 x hidden_dim 
        Query = self.MLP(self.flatten(Query)).unsqueeze(1)
        ...
        return self.softmax(probablity_dist/self.temp).squeeze(2)
